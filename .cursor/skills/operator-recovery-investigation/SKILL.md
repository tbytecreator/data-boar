...
2. **Try cheap tools first** — list dirs, `grep`, `git log`, PDF text extract.
...
- [ ] **Windows filename search:** `scripts/es-find.ps1` or operator `es.exe` when the name is unknown (session keyword `es-find`).
