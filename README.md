...
**Roadmap focus:** deeper **media** and **document** “hidden ingredient” coverage—including a **Tier 3b / Tier 4** taxonomy of embedded trackers, web bugs, stego-adjacent signals, and document-layer tricks in [docs/plans/PLAN_ADDITIONAL_DATA_SOUP_FORMATS.md](docs/plans/PLAN_ADDITIONAL_DATA_SOUP_FORMATS.md) (see [COMPLIANCE_AND_LEGAL.md](docs/COMPLIANCE_AND_LEGAL.md) for what is **shipped vs phased**); **cloud object storage** (S3-class, Azure Blob, GCS) as **planned** connectors in [docs/plans/PLAN_OBJECT_STORAGE_CLOUD_CONNECTORS.md](docs/plans/PLAN_OBJECT_STORAGE_CLOUD_CONNECTORS.md); **enterprise** back-office connectors, **crypto/controls** validation, **detection** refinement, **scan-complete / out-of-band notifications** (multi-channel, optional tenant copy; **off** by default), and **dashboard/API access** patterns for subscription- or shared-network deploys. Operators can read a **`runtime_trust`** snapshot on **`GET /status`** to see whether the running environment/configuration looks **trusted, degraded, or untrusted** for automation and lab workflows. Alignment with **ISO/IEC 27701** (PIMS), **SOC 2**, and **FELCA** (minor-data mapping) is documented; we continue to extend auditable and regional coverage. **U.S. child- and minor-privacy** contexts (optional sample profiles for **FTC COPPA**, **California AB 2273**, **Colorado CPA / under-18** framing) support **technical** audit prep and DPO-led review only—**not** legal advice or age proof; see [COMPLIANCE_FRAMEWORKS](docs/COMPLIANCE_FRAMEWORKS.md) and [Compliance and legal](docs/COMPLIANCE_AND_LEGAL.md).
