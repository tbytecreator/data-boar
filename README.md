...
**Why it holds up:** One engine for many sources; **config-driven** tuning (regex, ML/DL terms, norm tags, recommendation overrides) so you can align with different frameworks without code changes; **Excel reports** and **heatmaps** with trends across sessions; **schedulable** scans via API for continuous monitoring. We already support **LGPD**, **GDPR**, **CCPA**, **HIPAA**, and **GLBA** out of the box and provide **sample configuration and config-file examples** (e.g. [regex overrides](docs/regex_overrides.example.yaml), recommendation overrides in [USAGE](docs/USAGE.md)) so you can extend to **UK GDPR**, **PIPEDA**, **POPIA**, **APPI**, **PCI-DSS**, or custom norms. If your regulation or compliance scope needs specific tuning, **we can assist**—by creating tailored config files or making small code-side adjustments—to achieve better compatibility when you reach out. **Configurable timeouts** are supported: global `timeouts.connect_seconds` and `timeouts.read_seconds` (defaults 25 and 90) with per-target overrides so one slow database or API does not block the whole run. **Roadmap** priorities include security hardening, additional compliance samples (UK GDPR, PIPEDA, POPIA, APPI, PCI-DSS), optional scanning inside compressed archives, stronger crypto and controls validation, and SAP and other enterprise data sources (e.g. ServiceNow, Salesforce)—so the product keeps evolving toward the controls your auditors expect.
