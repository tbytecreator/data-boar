...
| ---- | ----------------- | ---------------- |
| **TLS / `unifi.local` vs IP** | O SAN do certificado bate com **hostname**; **HTTPS pelo IP** quebra validação (`WRONG_PRINCIPAL`). | Manter **`hosts`** / DNS; URL base da API = `https://unifi.local` no `.env`. |
...
| ---------- | ---- | -------- |
| **API Network Integration** | `.env` em `docs/private/homelab/` (ex.: `.env.api.udm-se.local`), `LAB_UDM_API_BASE_URL=https://unifi.local`, TLS de confiança. | Chave API **nunca** no git; rodar se exposta. |
...
2. **Retenção + disco:** definir dias; evitar encher discos pequenos em portáteis.
3. **Fase E opcional:** **Wazuh** manager numa **VM com RAM suficiente**; agentes em Linux do lab — **depois** dos logs centralizados.
