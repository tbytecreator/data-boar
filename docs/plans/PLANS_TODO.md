...
**Dashboard web surface cluster:** [#86](https://github.com/FabioLeitao/data-boar/issues/86) (RBAC) and [PLAN_DASHBOARD_I18N.md](completed/PLAN_DASHBOARD_I18N.md) (locale) share `api/routes.py` / templates. **Order:** **M-LOCALE-V1** ✅ (locale prefix on **`main`**, **2026-04**); **D-WEB** ✅; **maturity POC ready** (smoke + manual runbook) **then** **#86 Phase 1** on its own branch/PR — see [SPRINTS_AND_MILESTONES.md](SPRINTS_AND_MILESTONES.md) §4.2 and the **Integration** bullet *Maturity self-assessment* above.
...
| Scope import from exports (inventory bootstrap) | Optional: canonical schema; merge-safe config fragments                                                                              | Partial (generic CSV → YAML fragment on `main`) | Offline exports (e.g. Live Optics, GLPI, Nagios/Cacti-class, Prometheus/Datadog/Dynatrace **exports** where available) → Data Boar target hints; early scan rounds for consultants/customer teams. Complements PLAN_OPT_IN_NETWORK_PORT_SERVICE_HINTS. See PLAN_SCOPE_IMPORT_FROM_EXPORTS. |
