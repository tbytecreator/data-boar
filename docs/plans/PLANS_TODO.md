...
### Integration / active threads — last refreshed **2026-04-27**

- **Engineering doctrine consolidation (Slices 1–3 ✅ on `main`; Slice 4 governance refresh in PR):** Five normative manifestos shipped in Slice 1 under [`docs/ops/inspirations/`](../ops/inspirations/) — [THE_ART_OF_THE_FALLBACK.md](../ops/inspirations/THE_ART_OF_THE_FALLBACK.md), [DEFENSIVE_SCANNING_MANIFESTO.md](../ops/inspirations/DEFENSIVE_SCANNING_MANIFESTO.md), [ENGINEERING_BENCH_DISCIPLINE.md](../ops/inspirations/ENGINEERING_BENCH_DISCIPLINE.md), [INTERNAL_DIAGNOSTIC_AESTHETICS.md](../ops/inspirations/INTERNAL_DIAGNOSTIC_AESTHETICS.md), [ACTIONABLE_GOVERNANCE_AND_TRUST.md](../ops/inspirations/ACTIONABLE_GOVERNANCE_AND_TRUST.md). Slice 2 added the Sysinternals-style RCA block to `python -m cli.reporter` (`cli/reporter.py`) — every failed regen names the failed step + narrowed hypothesis + reproducible next command on **stderr**, leaving stdout clean for piped Markdown; pinned by [`tests/test_cli_reporter_rca.py`](../../tests/test_cli_reporter_rca.py). Slice 3 fused `pro/worker_logic.py::basic_python_scan` from two passes to one and skipped the redundant per-chunk string-coercion copy in `pro/engine.py::process_chunk_worker`; precision contract (CPF / email / Luhn / hard sample caps / statement timeouts) unchanged and pinned by [`tests/test_basic_python_scan_single_pass_parity.py`](../../tests/test_basic_python_scan_single_pass_parity.py). Slice 4 closes the loop in [BENCHMARK_EVOLUTION.md](BENCHMARK_EVOLUTION.md) §8: **0.574x recorded as the current technical debt baseline** (no Rust extension on lab host; Pro path takes ~1.74x more time than OpenCore in the 200k profile) — see [PLAN_ENGINEERING_DOCTRINE_CONSOLIDATION.md](PLAN_ENGINEERING_DOCTRINE_CONSOLIDATION.md). **Zero impact on customer DB locks** across all four slices.
