...
### Integration / WIP — last refreshed **2026-04-08**
...
- **Data soup narrative (WIP, docs only):** “Hidden / cloaked / legacy / forgotten” ingredients + **stacked opt-in sniffing** (file heuristics, [export breadcrumbs](PLAN_SCOPE_IMPORT_FROM_EXPORTS.md), [allowlisted port hints](PLAN_OPT_IN_NETWORK_PORT_SERVICE_HINTS.md)) — **product positioning**; **not** a legal, regulatory, or completeness guarantee.
...
- **Scope import / inventory bootstrap (roadmap):** Not implemented — phased plan [PLAN_SCOPE_IMPORT_FROM_EXPORTS.md](PLAN_SCOPE_IMPORT_FROM_EXPORTS.md): canonical import format → merge-safe **config fragments** from **offline exports** (assessment tools such as Live Optics when used as a provider, ITSM such as GLPI, classic monitoring such as Nagios/Cacti, observability breadcrumbs from Prometheus/Datadog/Dynatrace-class **exports**); **v1 may start from a manual CSV** of assets the client can list. **Sequencing:** after [GRC maturity POC](PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md) / DOCX→YAML track; before treating **#86** RBAC as the only dashboard front — see PLAN_SCOPE § *Operator sequencing*. Complements opt-in port hints (**passive seed** vs **active** probes). Positions Data Boar closer to **customer teams and existing toolchain** without replacing CMDB or observability stacks.
- **Maturity self-assessment / LGPD questionnaire (operator artefacts → product backlog):** [PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md](PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md) — **POC A on `main`:** gated `GET`/`POST /{locale}/assessment` (flag + tier), optional YAML pack, **SQLite** answers, optional **HMAC** row sealing + **`GET /status`** / **`--export-audit-trail`** integrity summary — **no** bundled proprietary questionnaire text in public Git. **Scoring + export ✅** — YAML **`scores`** (rubric), **%** on post-submit summary, **`GET /{locale}/assessment/export`** (CSV/Markdown **attachment**; USAGE describes download vs disk). **Per-batch history table on `GET /{locale}/assessment` ✅** (see [ADR 0032](../adr/0032-maturity-assessment-batch-history-sqlite.md)). **POC ready** checklist + **tier/JWT slice sketch** (orthogonal to **#86**): [PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md](PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md) § *POC ready* / § *Next slice*. **Next POC steps:** **tier / JWT alignment** with commercial docs, **RBAC-scoped history** when **[#86](https://github.com/FabioLeitao/data-boar/issues/86)** identity exists — same plan; doc hooks already point to Phase 0 route matrix. **[GitHub #86](https://github.com/FabioLeitao/data-boar/issues/86) Phase 1** (session + passwordless on `/{locale}/…`) ships on a **separate branch/PR**, **ASAP** **after** the maturity POC work **in flight** is brought to a coherent stop—do **not** block POC closure on #86. **Also:** DOCX→YAML curation workflow; then **annex/bundle** with technical report or consultant UX. Align **tier JWT enforcement** with [PLAN_PRODUCT_TIERS_AND_OPEN_CORE.md](PLAN_PRODUCT_TIERS_AND_OPEN_CORE.md) (Phases **1+**). **Related:** technical **PDF GRC** report for scan findings — [PLAN_PDF_GRC_REPORT.md](PLAN_PDF_GRC_REPORT.md) (planned, not the org form). Private: `docs/private/raw_pastes/general/LGPD_DOCS/`. **Commercial / subscription** feature—not open core.
...
| -                                                        | -----------                                      | ----                  | ----                                                                                 | ------------------------                                                                 |
| [86](https://github.com/FabioLeitao/data-boar/issues/86) | Reports / dashboard access by role or permission | Feature + security UX | [PLAN_DASHBOARD_REPORTS_ACCESS_CONTROL.md](PLAN_DASHBOARD_REPORTS_ACCESS_CONTROL.md) | `[H2][U2]` after **Priority band A**; **Phase 0 (D-WEB)** ✅ route matrix + middleware diagrams in plan; **Phase 1** = session + **passwordless (WebAuthn; Bitwarden Passwordless.dev minimum)**; **Phase 2** = RBAC; **Phase 3** = enterprise SSO/OIDC optional |

**Dashboard web surface cluster:** [#86](https://github.com/FabioLeitao/data-boar/issues/86) (RBAC) and [PLAN_DASHBOARD_I18N.md](completed/PLAN_DASHBOARD_I18N.md) (locale) share `api/routes.py` / templates. **Order:** **M-LOCALE-V1** ✅ (locale prefix on **`main`**, **2026-04**); **D-WEB** ✅. **#86 Phase 1** (session/passwordless) **ASAP on its own branch/PR** **after** the **maturity POC** slice in progress (history / tiers / RBAC alignment per product) reaches a deliberate checkpoint—see [SPRINTS_AND_MILESTONES.md](SPRINTS_AND_MILESTONES.md) §4.2 and the **Integration** bullet *Maturity self-assessment* above.
...
- **Defer optional or heavy work:** Version check, Selenium QA, Synthetic data, Notifications (later phases), SAP connector, **M-LOCALE-PLUS** (extra dashboard locales), and remaining **Additional data soup** items (Tier 1 formats, stego) come after core security and scan/report features or when more usage budget is available.
...
**Deferred (larger or later):** Secrets Phase B, Version check & self-upgrade (incl. optional Phase 9: .deb/apt repo, signed packages, bytecode-only install, winget-like), Selenium QA, Synthetic data, SAP connector, **M-LOCALE-PLUS** (extra dashboard locales). **Backlog:** Additional data soup **Tier 1** + **stego** + **Tier 3b** embedded-tracker heuristics ([PLAN_ADDITIONAL_DATA_SOUP_FORMATS.md](PLAN_ADDITIONAL_DATA_SOUP_FORMATS.md)); **Tier 3 rich media** is on **`main`** (see **Integration / WIP**).
...
| Doc snapshot per release                             | ✅ Baseline                     | `docs/releases/X.Y.Z.md` per version; **latest published: 1.7.0** (`docs/releases/1.7.0.md`, GitHub + Hub **2026-04-17**); next in-repo bump when ready: **1.7.1**. Optional “frozen bundle” → backlog.                                                                                                                   |
