...
- **Maturity self-assessment / LGPD questionnaire (operator artefacts → product backlog):** [PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md](PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md) — **POC A:** gated dashboard `GET /{locale}/assessment` **placeholder** (flag + tier) can ship on **`main`**; **no** bundled questionnaire text, **no** persistence yet. **Next:** SQLite + YAML import + content pack. **After** this track, align **tier JWT enforcement** with [PLAN_PRODUCT_TIERS_AND_OPEN_CORE.md](PLAN_PRODUCT_TIERS_AND_OPEN_CORE.md) (Phases **1+** still ahead). **Related:** technical **PDF GRC** report for scan findings — [PLAN_PDF_GRC_REPORT.md](PLAN_PDF_GRC_REPORT.md) (planned, not the org form). Private: `docs/private/raw_pastes/general/LGPD_DOCS/`. **Commercial / subscription** feature—not open core.
