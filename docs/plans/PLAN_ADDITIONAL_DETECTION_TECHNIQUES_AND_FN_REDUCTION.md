...
10. **Optional "incomplete data" mode**: config flag to lower aggregated_min_categories when coverage is partial and add report note that all flagged groups should be confirmed by a human.
11. **Optional single high-risk category "suggested review"**: when a group has only one category but it is high-risk (e.g. health), still produce a row with "suggested review" so the operator sees it.
