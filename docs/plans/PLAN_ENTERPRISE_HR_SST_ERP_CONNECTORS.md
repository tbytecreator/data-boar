...
2. **Read-only and least privilege:** Same as databases—credentials with minimal read scope; document required roles per vendor.
3. **Separation of concerns:** **Detection** (what might be sensitive) vs **legal qualification** (final Art. 9 LGPD assessment remains with DPO/counsel). Reports should use **conditional wording** (“possible special-category / health-like field”) where appropriate.
4. **Vendor reality:** Each product exposes **different** surfaces—REST/OData, SOAP, proprietary SDK, **read-only SQL replica**, CSV/Excel **exports** dropped on a share (already scannable via filesystem). No one-size-fits-all; plan per **category** then per **priority vendor**.
5. **URM / productivity tools:** Treat as **separate tier**—highest controversy; may be limited to **config + policy checklist** in reports unless the customer explicitly enables and has legal basis.
