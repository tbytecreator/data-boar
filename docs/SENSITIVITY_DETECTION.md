...
1. **ML** – TF-IDF + RandomForest trained on a list of **(text, label)** terms (sensitive vs non-sensitive). Terms come from a file or from inline config.
...
| `detection.quasi_identifier_mapping`          | list    | **[]**   | Optional list of `{ column_pattern, category }` or `{ pattern_detected, category }` to map columns/patterns to `gender`, `job_position`, `health`, `address`, `phone`, `other`. Built-in defaults map common names (e.g. gender, sex, cargo, department, health, address), **phone** (phone, telefone, celular, mobile, home phone, téléphone, handynummer, etc.), and **name/identifier** to `other` (first name, last name, surname, full name, birth name, nickname, nome, sobrenome, apellido, prénom, nom de famille, vorname, nachname, etc.). |
