...
| `sensitivity_detection.connector_format_id_hint`                | Boolean, default **false**. When **true**, connectors pass declared SQL types (e.g. `VARCHAR(11)`, `BIGINT`) into the detector. If there is **no regex match** and combined ML/DL confidence is **below** the MEDIUM threshold, conservative schema hints may elevate to **MEDIUM**: (a) `CHAR`/`VARCHAR` ID-like lengths → `FORMAT_LENGTH_HINT_ID` (includes **CPF/CNPJ/SSN** digit lengths and **UUID**-shaped lengths **32** / **36** when the column name contains `uuid`, `guid`, or `uniqueidentifier`); (b) integer-like types (`INT`, `BIGINT`, `NUMERIC(...,0)`) with ID-like names → `FORMAT_TYPE_HINT_ID_INT`; (c) email-typical declared lengths (`128/191/254/255/256/320`) with email-like names (`email`, `e_mail`, `mail`) → `FORMAT_LENGTH_HINT_EMAIL`. The **REST/API** connector infers `VARCHAR(n)` / `BIGINT` from JSON scalar samples when possible. **Off by default.** See `tests/test_format_length_hint.py`, `tests/test_rest_connector_format_hint.py`, Plan §4 in `PLAN_ADDITIONAL_DETECTION_TECHNIQUES_AND_FN_REDUCTION.md`. |                                                                              |
