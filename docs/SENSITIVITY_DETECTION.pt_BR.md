...
| `sensitivity_detection.connector_format_id_hint`                | Booleano, padrão **false**. Se **true**, os conectores passam tipos SQL declarados (ex.: `VARCHAR(11)`, `BIGINT`) ao detector. Sem regex e com confiança ML/DL **abaixo** do limiar MEDIUM, dicas conservadoras de schema podem elevar para **MEDIUM**: (a) tamanhos `CHAR`/`VARCHAR` ID-like → `FORMAT_LENGTH_HINT_ID` (inclui comprimentos de dígitos **CPF/CNPJ/SSN** e comprimentos tipo **UUID** **32** / **36** quando o nome da coluna contém `uuid`, `guid` ou `uniqueidentifier`); (b) tipos inteiros (`INT`, `BIGINT`, `NUMERIC(...,0)`) com nomes ID-like → `FORMAT_TYPE_HINT_ID_INT`; (c) tamanhos típicos de e-mail (`128/191/254/255/256/320`) com nomes de e-mail (`email`, `e_mail`, `mail`) → `FORMAT_LENGTH_HINT_EMAIL`. **Desligado por padrão.** Ver `tests/test_format_length_hint.py` e o plano §4 em `PLAN_ADDITIONAL_DETECTION_TECHNIQUES_AND_FN_REDUCTION.md`. |                                                                                |
