...
| ---         | -----                                                                                                                                                                                                                                  | ------    |
| **Phase A** |                                                                                                                                                                                                                                        |           |
| A1          | Add `pass_from_env` / `password_from_env` (and equivalent for every connector secret) in config schema and loader; resolve from env when present. Document in USAGE and SECURITY.                                                      | ⬜ Pending |
| A2          | Redact secret values when serving GET /config (parse → replace secrets with placeholder → serialize). Ensure POST /config does not overwrite secrets with placeholder when user saves; merge or keep refs.                             | ⬜ Pending |
| A3          | Document config file permissions, .gitignore for config.yaml and vault file, and “do not commit secrets”; add to SECURITY.md and release checklist.                                                                                    | ⬜ Pending |
| **Phase B** |                                                                                                                                                                                                                                        |           |
...
Each step should be tested (unit tests for loader resolution; integration test for redaction and re-import) and must not break existing configs that use inline secrets.

---

*Last updated: plan created. Update this doc when completing steps or when vault/alternatives change.*
