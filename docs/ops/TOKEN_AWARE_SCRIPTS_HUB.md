...
| `completao-chat-starter.ps1` | Print minimal Cursor chat lines (**`completao`** + **`tier:…`**) + suggested command | **`COMPLETAO_OPERATOR_PROMPT_LIBRARY.md`**, **`LAB_COMPLETAO_FRESH_AGENT_BRIEF.md`** (full blocks A–E when prose changes) |
