...
3. **Optional container image preflight (read-only):** manifest root **`completaoImageRefs`** — JSON array of strings (e.g. **`fabioleitao/data_boar:lab`**). Before host smoke, the script SSHs to **`completaoImageProbeSshHost`** if set, otherwise the **first** manifest **`sshHost`**, and runs **`docker image inspect`** / **`podman image inspect`** for each ref (**no pull**, **no create** — idempotent checks only). If any image is missing, the run **throws** after writing a **`failed`** JSON line. Opt out: **`-SkipImagePreflight`**. Pair with **`completaoDataContractsPath`** when both DB shape and runtime image must match a gate.
4. **Hybrid path:** **`lab-completao-orchestrate-hybrid-v173.ps1`** writes **`completao_hybrid_<timestamp>_events.jsonl`** in the same reports folder and skips **`docker run` / `podman run`** when the image is absent (same inspect pattern per node).
...
| **1** | Infrastructure / permission / reachability | SSH probe or transport failure, **`lab_completao_data_contract_check.py`** DB URL env empty or DB **`database error:`** connectivity/auth, orchestrator **`degraded`** or **`completed_with_skips`**, image preflight SSH failure. |
