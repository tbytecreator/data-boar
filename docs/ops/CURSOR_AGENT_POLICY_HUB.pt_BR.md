...
**Chat novo / pouco contexto:** começar em **[OPERATOR_AGENT_COLD_START_LADDER.pt_BR.md](OPERATOR_AGENT_COLD_START_LADDER.pt_BR.md)** ([EN](OPERATOR_AGENT_COLD_START_LADDER.md)) — escada ordenada num ecrã, router de tarefas e cinco regras inegociáveis; depois este hub ou o [`AGENTS.md`](../../AGENTS.md) conforme a necessidade.
...
| **Browser Cursor / SSO / separadores** (incl. UI **Docker Hub**, login **Google**) | [`.cursor/rules/cursor-browser-social-sso-hygiene.mdc`](../../.cursor/rules/cursor-browser-social-sso-hygiene.mdc) · [`.cursor/rules/operator-browser-warm-session.mdc`](../../.cursor/rules/operator-browser-warm-session.mdc) · [`.cursor/skills/cursor-browser-social-session/SKILL.md`](../../.cursor/skills/cursor-browser-social-session/SKILL.md) |
...
| **Completão no lab** (orquestração SSH, **`-Privileged`**) | [`.cursor/rules/lab-completao-workflow.mdc`](../../.cursor/rules/lab-completao-workflow.mdc) (**sudoers restrito**: validar só **`sudo -n`** + caminhos/flags exatos dos scripts; **`visudo -cf`** é administração e **não** é teste de NOPASSWD; assumir LAB/WSL aplicado salvo logs provarem o contrário) · [`LAB_COMPLETAO_RUNBOOK.md`](LAB_COMPLETAO_RUNBOOK.md) · palavra de sessão **`completao`** |
