...
3. **Preflight opcional de imagem de contentor (só leitura):** chave na raiz do manifesto **`completaoImageRefs`** — array JSON de strings (ex.: **`fabioleitao/data_boar:lab`**). Antes do smoke por host, o script faz SSH a **`completaoImageProbeSshHost`** se existir, senão ao **primeiro** **`sshHost`** do manifesto, e corre **`docker image inspect`** / **`podman image inspect`** por referência (**sem pull**, **sem criar** — só verificação idempotente). Se faltar imagem, a corrida **falha** após escrever linha JSON **`failed`**. Desligar: **`-SkipImagePreflight`**. Combina com **`completaoDataContractsPath`** quando o gate exige esquema de BD e imagem alinhados.
4. **Caminho híbrido:** **`lab-completao-orchestrate-hybrid-v173.ps1`** escreve **`completao_hybrid_<timestamp>_events.jsonl`** na mesma pasta de relatórios e **não** corre `docker run` / `podman run` se a imagem não existir no nó (mesmo padrão de `inspect`).
...
| **1** | Infra / permissão / alcance | Falha de *probe* SSH ou transporte, URL de BD em variável de ambiente vazia ou **`database error:`** de conectividade/auth em **`lab_completao_data_contract_check.py`**, orquestrador **`degraded`** ou **`completed_with_skips`**, falha de SSH no *preflight* de imagem. |
