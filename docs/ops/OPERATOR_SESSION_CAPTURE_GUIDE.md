...
2. **Update** the smallest set of files (prefer **one** new private note + **optional** one tracked cross-link).
3. **Never** commit LAN IPs, hostnames, family names tied to institutions, or commercial numbers—**private** or redacted.
4. If the idea is a **feature slice**, suggest a **`PLANS_TODO`** row or plan file update **separately** (do not merge unrelated topics into one giant private dump).
