...
2. **Inspirations:** open **[INSPIRATIONS_HUB.md](inspirations/INSPIRATIONS_HUB.md)** — confirm **TiddlyTools.com** loads; adjust link if needed.
3. **Supere recovery:** next paste or **`burst-showcase`** when you have energy — **SiteMap** / Nerd backlog per **`RESUME_TOPICS_JOURNAL`**.
4. **Deps:** triage **Dependabot** PRs when you want a **`deps`** session — not a moral obligation today.
5. **SRE calm:** one **`check-all`** or **`lint-only`** only if you’re about to commit — not as self-punishment.
