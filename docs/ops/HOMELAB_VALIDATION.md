...
| **Host firewall (Linux)** | Allow **inbound** **8088/tcp** from **your LAN subnet only** (example pattern, adjust CIDR: `sudo ufw allow from 192.168.0.0/16 to any port 8088 proto tcp comment 'Data Boar lab LAN'`). More context: [LMDE7_T14_DEVELOPER_SETUP.pt_BR.md](LMDE7_T14_DEVELOPER_SETUP.pt_BR.md). |
