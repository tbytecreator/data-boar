...
**Release date:** pending final publish (`v1.6.7` not tagged yet).

**Version string policy:** working tree uses **`1.6.7-rc`** while unreleased; at final publish, promote artifacts to **`1.6.7`** (tag + GitHub Release + Docker Hub) per [VERSIONING.md](../VERSIONING.md).
...
- **From source (pre-release):** `uv sync` at **`1.6.7-rc`** on `main` until publish.
