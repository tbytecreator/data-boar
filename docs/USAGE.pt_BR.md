...
| `--export-audit-trail`  | *(caminho opcional)* | Exporta trilha de auditoria em JSON a partir do SQLite (`data_wipe_log`, resumo de sessões, `dashboard_transport`). Sem caminho ou com `-` → **stdout**; caso contrário grava no arquivo. **Não** altera o banco. Incompatível com `--web` e `--reset-data`.                             |
...
- **Autoavaliação (POC):** `http://<host>:<port>/en/assessment` — placeholder quando **`api.maturity_self_assessment_poc_enabled`** está ligado; **404** se desligado. **Tier:** `licensing.effective_tier` no YAML para lab, ou claim **`dbtier`** no JWT com `licensing.mode: enforced` (**Pro+** para esta funcionalidade). Opcional **`api.maturity_assessment_pack_path`** para YAML do questionário (`core/maturity_assessment/pack.py`). Opcional **`DATA_BOAR_MATURITY_INTEGRITY_SECRET`** (ou **`api.maturity_integrity_secret_from_env`**) para **HMAC-SHA256 por linha** (integridade / anti-adulteração casual; **não** é criptografia). **`GET /status`** expõe **`maturity_assessment_integrity`** para demonstrar o resumo. Sem texto proprietário no repositório público.
