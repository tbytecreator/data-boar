...
- **Resiliência na amostragem SQL (alvos `type: database`):** o SQL gerado começa com o comentário de linha **`-- Data Boar Compliance Scan`** para rastreio em visões do motor. No alvo você pode definir **`sample_statement_timeout_ms`** ou **`sample_statement_timeout_seconds`** (**`0`** desliga); se omitido, o conector usa **5000 ms** por padrão. Esse orçamento aciona **`OPTION (MAX_EXECUTION_TIME=…)`** no SQL Server, hint **`MAX_EXECUTION_TIME`** no MySQL e **`SET LOCAL statement_timeout`** no PostgreSQL (transação curta por amostra). A variável de ambiente **`DATA_BOAR_SAMPLE_STATEMENT_TIMEOUT_MS`** sobrescreve o valor. Opcional: **`inter_query_delay_ms`** (pausa entre colunas). Estimativa de tamanho de tabela para modo “tabela grande” vem só de **estatísticas de dicionário** (`connectors/sql_table_row_estimate.py`), sem **`COUNT(*)`** na tabela de negócio.
