...
- **WebAuthn (opcional — fase 1, API JSON):** Com **`api.webauthn.enabled: true`**, defina a variável de ambiente indicada por **`api.webauthn.token_secret_from_env`** (padrão **`DATA_BOAR_WEBAUTHN_TOKEN_SECRET`**) **antes** de subir o processo; a falta do segredo faz o processo **encerrar na inicialização**. Habilita endpoints JSON **sem lock-in de vendor** (biblioteca open-source **`webauthn`** no PyPI) em **`/auth/webauthn/`** para registro e autenticação por passkey. **`api.require_api_key` não se aplica** a esses caminhos. Alinhe **`api.webauthn.origin`** e **`api.webauthn.rp_id`** com a URL do navegador (HTTPS recomendado). Credenciais ficam no SQLite (`webauthn_credentials`); **`--reset-data` / wipe** apaga essas linhas. O HTML do painel **ainda não** exige login por isso (seguimento **#86**). Ver **[ADR 0033](adr/0033-webauthn-open-relying-party-json-endpoints.md)** (EN).
