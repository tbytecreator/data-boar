...
- **`use_content_type` (opt-in):** Quando `true`, o scanner usa um helper de **magic bytes** para inferir o formato real do ficheiro (ex.: PDF renomeado como `.txt`) em filesystem e partilhas (SMB/WebDAV/SharePoint). Aumenta I/O e CPU por ficheiro; o padrão é só por extensão. **CLI:** `--content-type-check` força isso numa execução one-shot. **API/dashboard:** `content_type_check: true` no corpo de **`POST /scan`** ou a caixa no dashboard aplica só àquela varredura; o `config.yaml` gravado não é alterado. Detalhes e exemplos YAML: [USAGE.md](USAGE.md) (secção Global options / file_scan).
...
- `api` – porta da API; opcionalmente `require_api_key`, `api_key` ou `api_key_from_env` para exigir chave de API (cabeçalho X-API-Key ou Authorization: Bearer); GET /health permanece público. **Em produção recomenda-se** `require_api_key: true` e chave forte via variável de ambiente (ex.: `api.api_key_from_env: "AUDIT_API_KEY"`) para não armazenar a chave no config. Ver [SECURITY.md](../SECURITY.md).
  - Por padrão, quando você inicia a API via CLI (`python main.py --web ...`), o servidor web faz bind em **`127.0.0.1` (loopback)**, a menos que você defina explicitamente `api.host` no config (por exemplo, `0.0.0.0` ao rodar atrás de um proxy reverso ou dentro de um container). Em ambientes Docker ou Kubernetes, os manifests e exemplos já expõem a porta 8088 via bindings do container/Service; nesses casos, mantenha `api.host` no padrão ou use `0.0.0.0` apenas quando o container estiver protegido por política de rede, Ingress ou proxy reverso.
