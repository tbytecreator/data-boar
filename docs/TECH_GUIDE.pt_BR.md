...
- **CLI e API REST**: Execute auditoria única pela linha de comando ou inicie a API (porta padrão 8088) para `/scan`, `/start`, `/scan_database`, `/status`, `/report`, `/heatmap`, `/list`, `/reports/{session_id}`, `/logs` e `PATCH /sessions/{session_id}` para metadados tenant/technician. **WebAuthn opcional (fase 1):** com `api.webauthn.enabled` e segredo em ambiente (`DATA_BOAR_WEBAUTHN_TOKEN_SECRET` ou o nome em config), endpoints JSON em `/auth/webauthn/` implementam um **Relying Party** sem vendor obrigatório (biblioteca `webauthn` no PyPI) — [ADR 0033](adr/0033-webauthn-open-relying-party-json-endpoints.md) (EN); o login obrigatório no HTML do painel fica para o seguimento **#86**. As páginas **HTML** usam **prefixo de idioma** (ex.: `/en/`, `/pt-br/`); endpoints JSON permanecem sem prefixo — veja [USAGE.pt_BR.md](USAGE.pt_BR.md) (URLs principais / locale). Ambos os modos permitem marcar varreduras com informações opcionais de **tenant/customer** e **technician/operator**. **API key opcional** e **rate limiting** (veja [USAGE.pt_BR.md](USAGE.pt_BR.md)) protegem a API quando exposta. O dashboard web inclui **Help**, **About** (autor e licença) e cabeçalhos de segurança (veja [SECURITY.md](../SECURITY.md)). A aplicação funciona atrás de NAT, balanceadores de carga e proxies reversos (nginx, Traefik, Caddy); defina `X-Forwarded-Proto: https` quando o TLS for encerrado no proxy.
