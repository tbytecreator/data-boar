...
- **CLI and REST API**: Run one-shot audit from command line or start API (default port 8088) for `/scan`, `/start`, `/scan_database`, `/status`, `/report`, `/heatmap`, `/list`, `/reports/{session_id}`, `/logs`, and `PATCH /sessions/{session_id}` for tenant/technician metadata. **HTML** pages are served under a **locale path prefix** (e.g. `/en/`, `/pt-br/`); JSON endpoints stay unprefixed — see [USAGE.md](USAGE.md) (Web dashboard / locale). Both modes allow tagging scans with optional **tenant/customer** and **technician/operator** information. **Optional API key** and **rate limiting** (see [USAGE.md](USAGE.md)) protect the API when exposed. The web dashboard includes **Help**, **About** (author and license), and security headers (see [SECURITY.md](../SECURITY.md)). The application works behind NAT, load balancers, and reverse proxies (nginx, Traefik, Caddy); set `X-Forwarded-Proto: https` when TLS is terminated at the proxy.
