...
| **test_detector_entertainment_regression.py** | **Regression:** ML-only classification must not return ``HIGH`` + ``ML_DETECTED`` in lyrics / OSS Markdown / cifra-like contexts; patched ``predict_proba`` exercises ``ML_POTENTIAL_ENTERTAINMENT`` (see module docstring). Runs under ``check-all.ps1``.        |
