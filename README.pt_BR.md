...
Como um **javali**, ele é **faminto e tenaz**: não para de buscar, aprofunda em várias fontes e não fica na superfície. É resistente o bastante para ambientes heterogêneos. O Data Boar está pronto para **ingerir e digerir sua “sopa de dados”**—quaisquer que sejam os ingredientes: **idiomas**, **encodings** ou **regiões** diferentes na mesma base. Quando configurado e autorizado corretamente, pode varrer **arquivos locais e remotos**, **bancos SQL** (PostgreSQL, MySQL, SQL Server, Oracle, Snowflake e outros), **NoSQL** (MongoDB, Redis), **APIs**, **dashboards**, **Power BI**, **Dataverse**, **SharePoint**, **WebDAV**, **SMB/NFS** e outras fontes. Ele **não armazena nem exfiltra** conteúdo pessoal ou sensível—apenas **metadados** (onde foi encontrado, tipo de padrão, nível de sensibilidade). Assim você obtém **visibilidade para maturidade e remediação** sem mover nem copiar PII.

**Por que sustenta:** Um único motor para muitas fontes; ajuste **via configuração** (regex, termos ML/DL, norm tags, overrides de recomendação) para alinhar a diferentes frameworks sem mudar código; **relatórios Excel** e **heatmaps** com tendências entre sessões; varreduras **agendáveis** via API para monitoramento contínuo. Já oferecemos suporte direto a **LGPD**, **GDPR**, **CCPA**, **HIPAA** e **GLBA** e fornecemos **exemplos de configuração** (ex.: [regex overrides](docs/regex_overrides.example.yaml), overrides de recomendação no [USAGE](docs/USAGE.pt_BR.md)) para estender a **UK GDPR**, **EU GDPR**, **Benelux**, **PIPEDA**, **POPIA**, **APPI**, **PCI-DSS** ou normas customizadas. **Amostras de config para várias regiões** estão em [docs/compliance-samples](docs/compliance-samples/) (veja [frameworks de conformidade](docs/COMPLIANCE_FRAMEWORKS.pt_BR.md)); mais amostras regionais (ex.: Filipinas, Austrália, Singapura, UAE, Argentina, Quênia, Índia, Turquia) podem ser adicionadas no mesmo formato. **Seja qual for o idioma, encoding ou região da sua sopa de dados**, o javali está preparado: arquivos de config e de padrões podem usar **UTF-8** (recomendado), UTF-8 com BOM ou encodings legados (ANSI Windows, Latin-1), com o config principal lido por **auto-detecção** para não quebrar em ambientes mistos. Termos e relatórios podem seguir o idioma da sua região (ex.: EN+FR para Canadá, PT-BR+EN para Brasil, japonês ou árabe para APAC/MENA). Defina **`pattern_files_encoding`** quando usar arquivos de padrões em encoding diferente de UTF-8—veja [USAGE](docs/USAGE.pt_BR.md#file-encoding-config-and-pattern-files). Se sua regulamentação ou escopo exigir ajuste fino, **podemos ajudar**—configs sob medida ou pequenos ajustes no código—quando você entrar em contato. **Timeouts configuráveis** (globais e por alvo) evitam que uma fonte lenta trave a execução. As **prioridades de roadmap** incluem endurecimento de segurança, mais amostras de conformidade (EU GDPR, Benelux, PIPEDA, POPIA, APPI, PCI-DSS e opcionais regionais), varredura opcional dentro de arquivos compactados, validação de criptografia e controles e SAP e outras fontes empresariais—para o produto seguir evoluindo em direção aos controles que seus auditores esperam.
