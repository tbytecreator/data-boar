...
**Foco de roadmap:** cobertura mais profunda de **mídia** e **documentos** (“ingredientes ocultos”)—incluindo a taxonomia **Tier 3b / Tier 4** (rastreadores embutidos, web bugs, sinais adjacentes a estego e truques em camada de documento) em [docs/plans/PLAN_ADDITIONAL_DATA_SOUP_FORMATS.md](docs/plans/PLAN_ADDITIONAL_DATA_SOUP_FORMATS.md); veja [COMPLIANCE_AND_LEGAL.pt_BR.md](docs/COMPLIANCE_AND_LEGAL.pt_BR.md) para o que já **existe no produto** vs o que está **em fases futuras**. **Armazenamento de objetos em nuvem** (classe S3, Azure Blob, GCS) como conectores **planeados** em [docs/plans/PLAN_OBJECT_STORAGE_CLOUD_CONNECTORS.md](docs/plans/PLAN_OBJECT_STORAGE_CLOUD_CONNECTORS.md). Também: conectores **enterprise / back-office**, validação de **cripto/controles**, refinamento da **detecção**, **notificações** ao fim da varredura / fora de banda (vários canais, cópia opcional ao tenant; **desligadas por padrão**) e padrões de **acesso ao dashboard/API**. Operadores podem ler o snapshot **`runtime_trust`** em **`GET /status`** para saber se ambiente/config em execução parece **confiável, degradado ou não confiável** em automação e laboratório. O alinhamento com **ISO/IEC 27701** (PIMS), **SOC 2** e **FELCA** (mapeamento de dados de menores) está documentado; seguimos ampliando o suporte a normas auditáveis e regionais. Contextos **nos EUA** sobre privacidade de **crianças e menores** (perfis de amostra opcionais para enquadramento **FTC COPPA**, **Califórnia AB 2273**, **Colorado CPA / menores de 18**) apoiam preparação de auditoria e revisão liderada pelo DPO **em nível técnico** apenas—**não** são aconselhamento jurídico nem prova de idade; veja [COMPLIANCE_FRAMEWORKS.pt_BR.md](docs/COMPLIANCE_FRAMEWORKS.pt_BR.md) e [Conformidade e jurídico](docs/COMPLIANCE_AND_LEGAL.pt_BR.md).
