...
- **Snapper missing / `Unit snapper.service could not be found` / no `/.snapshots`:** The **`t14_snapper`** role only installs Snapper and runs **`snapper create-config`** when **root (`/`) is btrfs**. On **ext4** (typical default install), Ansible **cannot** create btrfs subvolumes or `/.snapshots` — that would require changing the root filesystem (reinstall with btrfs, or a planned migration), not a playbook-only fix. The role skips and prints a debug message. Snapper on Debian uses **`snapper-timeline.timer`** / **`snapper-cleanup.timer`**, not `snapper.service`. Set **`t14_snapper_enabled: false`** if you want to silence the role on ext4 hosts.
