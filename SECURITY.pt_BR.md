...
- **Notificações ao operador (webhooks):** O bloco opcional `notifications` pode enviar resumos de varredura para Slack, Teams, Telegram ou um URL genérico. Trate URLs de webhook e tokens de bot como segredos; use `${VAR_DE_AMBIENTE}` no YAML ou só variáveis de ambiente. Veja **`docs/USAGE.pt_BR.md`** (Notificações ao operador). O envio repete em falhas transitórias (HTTP 5xx / rede); isso **não** substitui TLS nem política de rede.
