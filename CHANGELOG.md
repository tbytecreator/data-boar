...
- **Dashboard auth (Phase 1a — WebAuthn JSON core):** Optional **`api.webauthn`** + **`DATA_BOAR_WEBAUTHN_TOKEN_SECRET`** enable **vendor-neutral** passkey JSON endpoints under **`/auth/webauthn/`** (PyPI **`webauthn`**), SQLite table **`webauthn_credentials`**, signed session cookie — [ADR 0033](docs/adr/0033-webauthn-open-relying-party-json-endpoints.md). Expanded pytest coverage (`tests/test_webauthn_rp.py`); operator subset **`scripts/smoke-webauthn-json.ps1`** + [SMOKE_WEBAUTHN_JSON.md](docs/ops/SMOKE_WEBAUTHN_JSON.md). Default **disabled**.
- **Dashboard auth (Phase 1b — HTML session + CSRF minimal):** When **at least one** passkey exists, locale-prefixed dashboard routes require a WebAuthn session cookie (**`GET /{locale}/login`** is public; **`help`** / **`about`** stay public). **`POST /{locale}/config`** and **`POST /{locale}/assessment`** verify a signed **CSRF** token when the gate applies. **`GET /{locale}/login`** exempt from global API key so operators can reach sign-in. Static **`api/static/webauthn-login.js`**. Tests: `tests/test_webauthn_html_gate.py`, `tests/test_html_csrf.py`.
- **Dashboard RBAC (Phase 2 — GitHub [#86](https://github.com/FabioLeitao/data-boar/issues/86)):** Optional **`api.rbac.enabled`** with **Pro+** `dashboard_rbac` enforces roles (`admin`, `dashboard`, `scanner`, `reports_reader`, `config_admin`) on API and locale HTML routes; **`webauthn_credentials.roles_json`** (JSON array) or **`default_roles`** / **`api_key_roles`**. Middleware runs after the WebAuthn HTML gate. **`GET /status`** / **`GET /health`** `enterprise_surface.access_surface.rbac` reports `enabled` vs `not_implemented`. Tests: `tests/test_rbac.py`. **Phase 3** (enterprise SSO/OIDC) remains future work.
- **Filesystem “data soup” Tier 1 + stego hints:** **`SUPPORTED_EXTENSIONS`** adds **`.epub`**, **`.parquet`**, **`.feather`**, **`.orc`**, **`.avro`**, **`.dbf`** with extractors in **`connectors/data_soup_formats.py`**; optional **`uv pip install -e ".[dataformats]"`** (PyArrow, fastavro, dbfread) for columnar/legacy types. **Opt-in** **`file_scan.scan_for_stego`**, CLI **`--scan-stego`**, dashboard **`scan_for_stego`**, and **`POST /scan`** body — appends a bounded byte-entropy hint line on image/audio/video (heuristic only). **Fix:** `connectors/filesystem_connector._read_text_sample` now imports **`RICH_MEDIA_SCAN_EXTENSIONS`** from **`core.rich_media_magic`** (the previous import from **`rich_media_sample`** was invalid and caused all extraction to fall back to empty content).
